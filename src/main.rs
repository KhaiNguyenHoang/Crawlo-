...
const MAP: usize = 20;
const T_SIZE: (f32, f32) = (32.0, 16.0);
...
fn bfs(
    ...
    while let Some((cx, cy)) = queue.pop_front() {
        ...
        for (dx, dy) in [(0, -1), (0, 1), (-1, 0), (1, 0)] {
            ...
            if visited[ny][nx] || map[ny][nx] != Tile::Floor {
                continue;
            }
...
impl Game {
    ...
    fn update(&mut self, dt: f32) -> bool {
        if self.hp <= 0 || self.monters.is_empty() {
            return true;
        }
        ...
        if !self.path.is_empty() {
            ...
            if self.player_cd <= 0. {
                ...
                if let Some(i) = self.monters.iter().position(|m| m.x == nx && m.y == ny) {
                    self.damege_monter(i, 10);
                    self.path.clear();
                } else {
                    self.path.remove(0);
                    self.px = nx;
        ...
        let occupied: Vec<_> = self
            ...
            .collect();
        for i in 0..self.monters.len() {
            self.monters[i].cd -= dt;
            if self.monters[i].cd <= 0. {
                self.monters[i].cd = 1.0;
                let (mx, my) = (self.monters[i].x, self.monters[i].y);
                ...
                if d == 1 {
                    ...
                    if path.len() > 1 && !occupied.contains(&(path[0].0, path[0].1)) {
                        self.monters[i].x = path[0].0;
                        self.monters[i].y = path[0].1;
                    }
                }
            }
        }
    ...
    fn damege_monter(&mut self, idx: usize, amount: i32) {
        self.monters[idx].hp -= amount;
        let (sx, sy) = to_screen(self.monters[idx].x, self.monters[idx].y, self.cam);
        self.texts.push(DmgText {
            x: sx,
            y: sy - 40.,
            dmg: amount,
        ...
        if self.monters[idx].hp <= 0 {
            self.monters.remove(idx);
            self.score += 50;
        }
    }
    ...
    fn draw(&mut self) {
        ...
        for m in &self.monters {
            draw_stickman(m.x, m.y, self.cam, true);
        }

        for t in &self.texts {
            if t.dmg < 0 {
                draw_text(&format!("+{}", -t.dmg), t.x, t.y, 20., GOLD);
            } else {
...
async fn main() {
    ...
    loop {
        ...
        match state {
            AppState::Menu => {
                draw_text("Menu - Enter to start", 100.0, 100.0, 40.0, BLACK);
